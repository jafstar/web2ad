...
export async function exportFinalAd(runId) {
  ...
  // Real production bug: ffmpeg-static's LINUX binary corrupts a
  // filtergraph segment that chains 2+ filters via comma within one
  // bracketed run (e.g. "scale=1024:1024,setsar=1[v0]") - it truncates
  // mid-way through, right after an inner filter's 2nd option, and drops
  // everything up to the next recognizable token. Confirmed via a
  // diagnostic route that neither Node's argv handling nor the actual file
  // content ffmpeg reads is corrupted - the bug is in ffmpeg's own
  // filtergraph string parser - and that a SINGLE, unchained filter per
  // bracketed segment always survived intact. Every filter below now gets
  // its own stage instead of chaining via comma; the two fades (previously
  // identical text "afade=t=out:st=X:d=1" for both music and narration)
  // also get distinct start/duration values as a second layer of safety.
  // See preview.js's renderPreview for the fuller writeup; never seen
  // locally since dev used the Windows ffmpeg-static build, which doesn't
  // have this bug.
  ...
  const filterComplex = `${scaleChains};${concatInputs}concat=n=${clipPaths.length}:v=1:a=0[vc];` +
    ...
    `[${musicIdx}:a]volume=0.3[as0];` +
    `[as0]afade=t=out:st=${musicFadeStart}:d=1[music];` +
