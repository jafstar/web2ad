...
export async function exportFinalAd(runId) {
  ...
  // a raw -filter_complex CLI argument - ffmpeg-static's LINUX binary was
  // found (live, in production) to silently corrupt long -filter_complex
  // argv strings with repeated tokens (same afade=t=out:st=<value> used
  // twice here, for music and narration). Confirmed via a diagnostic route
  // that Node's own argv handling is 100% intact for this exact string -
  // the corruption is inside ffmpeg's own CLI arg parser on this platform
  // build. See preview.js's renderPreview for the fuller writeup.
