...
export async function exportFinalAd(runId) {
  ...
  // Real production bug: ffmpeg-static's LINUX binary corrupts the
  // filtergraph when it contains two structurally identical option
  // strings - here both fades used the literal text "afade=t=out:st=X:d=1"
  // (X was the same fadeStart for both music and narration). Confirmed via
  // a diagnostic route that neither Node's argv handling nor the file
  // ffmpeg reads from is corrupted - the bug is in ffmpeg's own
  // filtergraph string parser. Giving the two fades distinct start/duration
  // values (music recedes fractionally before narration - inaudible)
  // removes the exact duplicate text and works around it. See
  ...
  const filterComplex = `${scaleChains};${concatInputs}concat=n=${clipPaths.length}:v=1:a=0[vc];` +
    ...
    `[${musicIdx}:a]volume=0.3,afade=t=out:st=${musicFadeStart}:d=1[music];` +
