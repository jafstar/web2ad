...
export async function exportFinalAd(runId) {
  ...
  // production retesting on the actual Vercel deployment (unreproducible
  // locally - dev uses the Windows ffmpeg-static build, which doesn't have
  // this bug). Two consistent, separate failures in ffmpeg-static's LINUX
  // binary's filtergraph parser:
  //   1) any filter given a 3rd colon-option truncates right after its 2nd
  //      option, regardless of whether it reads from a raw stream or an
  //      intermediate label (confirmed for zoompan in preview.js).
  //   2) with two separate afade filter instances anywhere in the graph,
  //      only the LAST one in the string ever parsed correctly - every
  //      attempt with 2 afade calls had exactly one of them corrupt.
  // Fix: only one afade call now, applied to narration+music AFTER mixing
  // instead of fading each independently before the mix (same audible
  // result - everything still fades out together near the end). tpad only
  // ever had 2 options (stop_mode, stop_duration) so it's left as-is -
  // matches the boundary that never failed. concat is left as direct
  // multi-input structurally (it inherently needs each clip's own
  // processed stream) - not yet proven affected by this bug; revisit here
  // first if export still fails after this change.
  ...
  const filterComplex = `${scaleChains};${concatInputs}concat=n=${clipPaths.length}:v=1:a=0[vc];` +
    ...
    `[${musicIdx}:a]volume=0.3[am];` +
    `[${narrationIdx}:a][am]amix=inputs=2:duration=longest:dropout_transition=0[mixed];` +
    `[mixed]afade=t=out:st=${fadeStart}:d=1[a]`
