...
export async function renderPreview({ brief, script }) {
  ...
  // a raw -filter_complex CLI argument. Real production bug found live:
  // ffmpeg-static's LINUX binary corrupts a filtergraph segment that
  // chains 2+ filters via comma within one bracketed run (e.g.
  // "scale=...,zoompan=...,scale=...,setsar=1[v]") - it truncates mid-way
  // through, right after an inner filter's 2nd option, and drops
  // everything up to the next recognizable token. Confirmed via a
  // diagnostic route that neither Node's argv nor the actual file content
  // ffmpeg reads is corrupted, and confirmed that a SINGLE, unchained
  // filter per bracketed segment (no comma) always survived intact across
  // every failed attempt - so every filter here now gets its own `;`
  // -separated stage with its own intermediate label instead of chaining
  // via comma. Never seen locally since dev used the Windows ffmpeg-static
  // build, which doesn't have this bug.
  ...
  fs.writeFileSync(filterComplexPath,
    `[0:v]scale=1280:1280[vs0];` +
    `[vs0]zoompan=z='min(zoom+0.0015,1.15)':d=${PREVIEW_SECONDS * 25}:s=1280x1280:fps=25[vs1];` +
    `[vs1]scale=1024:1024[vs2];` +
    `[vs2]setsar=1[v];` +
    `[2:a]volume=0.35[as0];` +
    `[as0]afade=t=out:st=${musicFadeStart}:d=0.6[music];` +
