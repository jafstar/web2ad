...
export async function renderPreview({ brief, script }) {
  ...
  // narrowed down through several rounds of production retesting on the
  // actual Vercel deployment (unreproducible locally - dev uses the
  // Windows ffmpeg-static build, which doesn't have this bug):
  // ffmpeg-static's LINUX binary's filtergraph parser is broken in two
  // specific, consistent ways -
  //   1) zoompan always truncates right after its 2nd option (z, d) no
  //      matter what else changed around it (comma-chained vs its own
  //      stage, intermediate label vs raw stream input) - every attempt
  //      with a 3rd zoompan option (s=, fps=) failed.
  //   2) with two separate afade filter instances anywhere in the graph,
  //      only the LAST one in the string ever parsed correctly, regardless
  //      of which one it was - every attempt with 2 afade calls had
  //      exactly one of them corrupt.
  // Fix: zoompan now only ever gets its first 2 options (z, d) - sizing
  // and framerate are handled by separate scale/fps filter stages instead.
  // And there's only one afade call total now - narration and music are
  // mixed first, then faded together as a single combined stream, instead
  // of each being faded independently before mixing (same audible result:
  // everything still fades out together near the end).
  const fadeStart = PREVIEW_SECONDS - 0.6
  const filterComplexPath = path.join(tmp, 'filter_complex.txt')
  const expectedFilterComplex =
    `[0:v]scale=1280:1280[vpre];` +
    `[vpre]zoompan=z='min(zoom+0.0015,1.15)':d=${PREVIEW_SECONDS * 25}[vz];` +
    `[vz]fps=25[vfps];` +
    `[vfps]scale=1024:1024[vs2];` +
    `[vs2]setsar=1[v];` +
    `[2:a]volume=0.35[am];` +
    `[1:a][am]amix=inputs=2:duration=first:dropout_transition=0[mixed];` +
    `[mixed]afade=t=out:st=${fadeStart}:d=0.6[a]`
  fs.writeFileSync(filterComplexPath, expectedFilterComplex)
  try {
    ...
    // TEMP diagnostic - every isolated/synthetic repro of this exact
    // filtergraph structure succeeds, so something about the REAL call
    // differs. Read the file back at the moment of failure to rule in/out
    // a write-race or wrong-content issue rather than trusting ffmpeg's
    // own (proven unreliable) error text.
    let diskContent = null
    try { diskContent = fs.readFileSync(filterComplexPath, 'utf8') } catch {}
    e.message = `${e.message}\n---DISK_CONTENT(${diskContent?.length})---\n${diskContent}\n---EXPECTED(${expectedFilterComplex.length})---\n${expectedFilterComplex}\n---MATCH---\n${diskContent === expectedFilterComplex}`
    throw e
  }
