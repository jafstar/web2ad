...
export async function renderPreview({ brief, script }) {
  ...
  //
  // Real root cause of tonight's "spawn ffmpeg ENOENT" / filtergraph
  // corruption saga: this was never an ffmpeg bug. PREVIEW_SECONDS is a
  // module-level literal constant, so `` `d=${PREVIEW_SECONDS * 25}` ``
  // chained together with several other template-literal segments via `+`
  // was compile-time constant-foldable - and Next's production minifier
  // (SWC) folded it into a single string literal WRONG, silently dropping
  // several middle segments. Confirmed by reading the compiled
  // .next/server output directly: the baked-in string literal was already
  // truncated before the app ever ran - ffmpeg was accurately reporting
  // the broken string it was actually given the whole time. Never
  // reproduced locally because local testing never went through the
  // production build/minify step. Building the string via array + join
  // instead of chained template-literal `+` avoids the pattern that
  // triggered the bad fold.
