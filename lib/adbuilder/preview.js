...
export async function renderPreview({ brief, script }) {
  ...
  // ffmpeg-static's LINUX binary silently corrupts long -filter_complex
  // argv strings containing repeated tokens (confirmed via a diagnostic
  // route: Node passes execFile's argv through 100% intact on this exact
  // string - proven both on a clean spawn and on execFile's own
  // "Command failed" error-message reconstruction - so the corruption is
  // inside ffmpeg's own CLI arg parser, not our code or the runtime.
  // -filter_complex_script goes through a different ffmpeg code path and
  // isn't affected. The Windows ffmpeg-static build used in local dev
  // never showed this, which is why it wasn't caught until real production
  // use tonight.
