...
export async function renderPreview({ brief, script }) {
  ...
  // ffmpeg-static's LINUX binary corrupts the filtergraph when it contains
  // two structurally identical option strings (both fades here used the
  // literal text "afade=t=out:st=4.4:d=0.6") - confirmed via a diagnostic
  // route that Node passes argv/writes the file 100% intact, then
  // confirmed switching to -filter_complex_script alone did NOT fix it
  // (same corruption reading from a real file), meaning the bug is in
  // ffmpeg's own filtergraph string parser, not in how the string reaches
  // it. Giving the two fades distinct start times (music recedes fractionally
  // before narration - inaudible, 50ms) removes the exact duplicate text
  // and works around it. Never seen locally since dev used the Windows
  // ffmpeg-static build, which doesn't have this bug.
  ...
  fs.writeFileSync(filterComplexPath,
    ...
    `[2:a]volume=0.35,afade=t=out:st=${musicFadeStart}:d=0.6[music];` +
