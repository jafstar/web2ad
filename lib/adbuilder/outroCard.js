...
// the mascot. Rendered as its own line; phoneNumber still gets its own
// line below that when present, so the two can coexist.
function outroMarkup(businessName, phoneNumber, brandColors = [], tagline = '') {
  ...
  const extraCss = `
    ...
    .outro-card { text-align: center; animation: outroIn 0.7s cubic-bezier(0.16, 1, 0.3, 1) forwards; max-width: 80%; }
    ...
    .outro-tagline { font-family: Arial, sans-serif; font-size: 22px; margin-top: 14px; color: #f4ede1; opacity: 0.85; line-height: 1.4; }
...
export async function buildOutroClip(businessName, phoneNumber, brandColors = [], tagline = '', { width = 1024, height = 1024 } = {}) {
  const { bodyHtml, extraCss } = outroMarkup(businessName, phoneNumber, brandColors, tagline)
