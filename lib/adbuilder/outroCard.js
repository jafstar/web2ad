...
// No real brand-color/logo extraction exists yet (would need a homepage
// screenshot + vision analysis - the Chromium infra this reuses makes
// that a real next step, not built yet). This warm dark palette is a
// deliberate, honest placeholder default, not a fake "brand color" -
// works reasonably across verticals until real per-business extraction
// exists.
function outroMarkup(businessName, phoneNumber) {
  ...
  const extraCss = `
    ...
    .outro-phone { font-family: Arial, sans-serif; font-size: 26px; margin-top: 16px; color: #e8a852; letter-spacing: 0.5px; }
    html, body { background: radial-gradient(circle at 50% 42%, #2a2118 0%, #14100b 100%) !important; }
  `
...
export async function buildOutroClip(businessName, phoneNumber, { width = 1024, height = 1024 } = {}) {
  const { bodyHtml, extraCss } = outroMarkup(businessName, phoneNumber)
