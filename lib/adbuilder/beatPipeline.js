...
export async function writeAdBeats(brief, direction = '') {
  const vertical = verticalConstraints(brief.vertical)
  // direction is the free-text "what do you want to see made" field from
  // step 1 - a real creative steer from the business owner (e.g. "end
  // with our cowboy mascot and the phone number", "keep it fast-paced").
  // Real instruction, not just flavor text, so it's framed as a
  // requirement to follow, not background color.
  ...
  const prompt = `${briefText(brief)}\n\nWrite the beat sequence now.${vertical.toneRegisterNote ? ` ${vertical.toneRegisterNote}` : ''}${directionNote}`
...
// Real, separate cheap path requested live 2026-07-26 for the new 3-step
// v2 funnel's step 2 ("display audio, narrator, and visual image for
// scene 1") - writes the REAL full beat sequence (so the story shown is
// the exact one that gets built later, not a different draft) but only
// renders the first beat's narration + still keyframe, same cost/speed
// tradeoff v1's renderPreview already makes (one image + one TTS call,
// no motion generation, no music, no compositing). previewRunId only
// namespaces this one Cloudinary upload - the real run gets its own id
// and regenerates every beat's media fresh in buildBeatAd below.
export async function previewFirstBeat(brief, direction = '') {
  console.log('[beatPipeline] Writing beat sequence (preview)...')
  ...
  console.log('[beatPipeline] Synthesizing first-beat narration...')
  const audioBuf = await synthesizeSpeech({ text: firstBeat.phrase, apiKey, voiceId: NARRATION_VOICE_ID })
  const narratorAudioUrl = `data:audio/mpeg;base64,${audioBuf.toString('base64')}`

  console.log('[beatPipeline] Generating first-beat scene image...')
  const previewRunId = `beatpreview-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`
  const { url: sceneImageUrl } = await generateKeyframeFor(previewRunId, firstBeat.id, firstBeat.visual, brief, atmosphere)

  return { beats, atmosphere, narratorAudioUrl, sceneImageUrl }
}
...
export async function composeBeatAd(runId, brief, beats) {
  ...
  try {
    ...
    const outro = await buildOutroClip(brief.businessName, brief.phoneNumber || null, brief.brandColors || [])
