...
// video engine itself.
export const RENDER_DURATION_SECONDS = 6
...
Pick a real STRUCTURE for this specific story - don't default to the same shape every time. Real options, genuinely different from each other:
- A single character's arc (want, obstacle, resolution) - only one of many valid shapes, not the default.
- An ensemble - several different people/moments, tied together by the same place, product, or idea, not one continuous protagonist.
- No character at all - the product, the process, or the place itself is the subject (hands at work with no single "character," a space at different times of day, an object examined closely).
- Second-person direct address - speaking straight to the viewer's own situation, not narrating someone else's.
- An observational montage - a rhythm of real, specific, disconnected-but-thematically-linked moments, not a plotted beginning/middle/end.
Choose whichever structure actually fits this business and the chosen angle best - that choice itself should vary business to business, not default to "one skeptical character converts."

Each beat needs exactly two lines:
PHRASE: a short natural spoken phrase - however long actually reads well in one breath (could be three words, could be fifteen; don't force a fixed length), the kind of thing that gets its own shot, not a sentence fragment cut mid-thought.
VISUAL: a one-sentence, concrete, filmable scene for that exact phrase (who/what is on screen, setting, action).

Read in order, the phrases must form ONE coherent piece with real rhythm and intention - not four unrelated stock clips with a voiceover layered on top. How the visuals connect to each other should follow from the structure you picked (a single continuous action's real coverage, a set of parallel moments, a rhythm of cuts) - don't default to the same hand-reaches-for-object chaining regardless of what the story actually is.
...
// Real, live-caught motion quality issue: the raw visual description was
// going straight to Hailuo with zero pacing guidance. AI video models are
// broadly weak at fast/complex anatomy - mouths and eating/chewing motion
// specifically are one of the worst cases (already known well enough to
// have shaped shots.js's own 'objects' framing option) - unguided, they
// tend to render eating as too fast and anatomically wrong ("sideways").
// This note only affects the MOTION call, not the keyframe/still image,
// since a still has no speed to get wrong.
export const MOTION_PACING_NOTE = ' Motion should be slow, natural, and true to how this actually happens in real life - no rushed, exaggerated, or unnatural movement. If the action involves eating, biting, or the mouth, keep it especially subtle and slow - a small, natural, deliberate bite, not an exaggerated or fast one.'
  ...
  if (brief.outroEnabled !== false) {
    try {
      ...
      const outro = await buildOutroClip(brief.businessName, brief.phoneNumber || null, brief.brandColors || [], brief.outroText || '', {
        logoDataUrl: brief.outroLogoDataUrl || null,
        websiteText: brief.outroWebsiteText || '',
        background: brief.outroBackground || 'themed',
      })
