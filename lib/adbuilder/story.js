...
export async function writeAdStory(brief, styleTag) {
  // Real gap closed 2026-07-26: this is the pipeline the actual production
  // wizard uses (writeAdStoryFast's vertical-aware tone register only
  // applied to the Playground's separate fast pipeline) - a high-trust
  // business running the real funnel with styleTag='funny' had zero
  // register protection until this. Same note injected at three points:
  // the initial draft (shapes the writer's baseline voice for ANY style
  // tag, not just funny), the Jester critique (the one step explicitly
  // hunting for a joke, most likely to push toward broad comedy against
  // the grain), and the lead edit (so incorporating notes doesn't erode
  // the register the draft/refine already established).
  const styleHint = STYLE_HINTS[styleTag]
  const verticalNote = verticalConstraints(brief.vertical).toneRegisterNote
  const draftBrief = `${briefText(brief)}\n\nWrite a short (35-45 word, fewer if it includes dialogue) real short story - not ad copy - with a beginning, middle, and end, that doubles as ad narration for this business. One clear throughline: pick ONE customer/character and follow them through one real moment (want/need -> they get it -> the payoff). End with a natural, ungimmicky mention of the business - it should feel like part of the story's own world, not a tacked-on slogan.${styleHint ? ` Style direction: ${styleHint}` : ''}${verticalNote ? ` ${verticalNote}` : ''}`
  ...
  const JESTER = `You are The Jester, a professional comedy writer. This ad is allowed to be genuinely funny. Find the ONE place a real joke, a surprising turn, or a funnier specific detail would make this more shareable - name the actual line and what should replace it. ${FOCUS}${verticalNote ? ` ${verticalNote}` : ''}`
  ...
  const visualGuidance = verticalConstraints(brief.vertical).visualGuidance
  const LEAD_SYSTEM = `You are the Lead Editor. Rewrite the draft into its final form using your judgment on the notes below (not every note demands a literal fix, but take them seriously) - preserve the core story, keep it 35-45 words (tighter if it has dialogue - attribution and dramatic pauses read slower than plain narration), must read well spoken aloud in under 20 seconds. Real stories often include quoted dialogue ("cut back," he said) - that's good, keep it if it's there, just budget fewer words for it.${verticalNote ? ` ${verticalNote}` : ''}

Also write a one-sentence, concrete, filmable visual description of the story's single key moment (who/what is on screen, setting, action) - this becomes an image-generation prompt, so it must not depend on any on-screen text, logo, or wordmark being legible.${visualGuidance ? ` This single visual line is exactly as visible to the audience as the full shot breakdown elsewhere in this pipeline, so the same real constraint applies to it: ${visualGuidance}` : ''}
...
VISUAL: <the one-sentence visual description>`
  ...
  final.trace = { styleTag: styleTag || null, vertical: brief.vertical || null, claudeDraft, refinedDraft: draft, notes: { arbiter, realist, visionary, jester } }
  return final
}
...
// Real, live-caught failure, tested against Cheat's Cheesesteaks on both
// writers: asking the model to "silently identify the cliche, then
// subvert it" in one breath doesn't reliably work. Gemini just ignored it
// and wrote a straight, reverent food-ritual story - same voice as
// Professional. Grok DID attempt real subversion ("it tasted cleaner than
// he remembered, which somehow made it worse") but it stayed subtle/dry,
// nowhere near the broad obvious comedy of a hand-written example like
// "apply wellness-supplement language to a cheesesteak." The difference:
// that example named its incongruous premise explicitly and concretely
// before a single line of story got written. Forcing the model to output
// that premise as its own visible field - not an invisible mental step it
// can skip - is the fix; PREMISE and STORY are parsed separately so the
// premise can also be handed to the editor step, which needs to know
// exactly what joke it's protecting from getting smoothed out.
// Real, live-caught SECOND failure mode, same session: forcing an
// explicit premise fixed the "no incongruity at all" bug, but the model's
// next default move was reaching for a generic borrowed pop-culture trope
// ("buying lunch like a spy on a mission") - technically incongruous, but
// it's ITSELF a cliche at this point (this exact "mundane errand played
// as an action movie" bit is one of the most worn tropes in advertising),
// disconnected from anything actually true about the business, and
// swappable onto literally any other food business unchanged. The
// versions that actually landed (spiritual enlightenment, engineering
// precision) both derived their elevated register from words already in
// the business's own real trust signals (chef-driven, precision,
// made-from-scratch). Grounding the premise in the business's own stated
// facts is what makes it feel specific and smart instead of generic; an
// explicit ban on the worn tropes is the more reliable guardrail than
// just hoping "specific" gets interpreted correctly on its own.
const FUNNY_WRITER_SYSTEM = `${WRITE_SYSTEM}

This ad must be genuinely funny - real structural irony, not a pun. Before writing, pick ONE concrete, specific incongruous premise, built this way: take a real word, phrase, or claim from THIS business's own description or trust signals (chef-driven, mindful, precision, artisan, made-from-scratch, whatever they actually say about themselves) and apply its full, most extreme, most literal register - completely straight-faced - to the mundane reality of actually getting/eating the product. The premise must only make sense for THIS specific business, not something swappable onto any other business unchanged - if you could paste the same premise onto a random other company's ad, pick a different one. Do NOT reach for a generic borrowed world (no spy/heist/action-movie missions, no reality-TV competition format, no courtroom drama) - those are their own tired cliches now, not real subversion. Name the actual mismatch in one sentence. Then write the story entirely inside that premise, played completely sincere and deadpan - never explain the joke, never use an exclamation point, never wrap it up with a punchline flourish. The friction between the stated premise and the ordinary/greasy/real reality underneath it IS the joke - let the reader cross that gap themselves.

Output in EXACTLY this format, nothing before or after, no markdown:
PREMISE: <the one-sentence concrete incongruous premise you picked>
STORY: <the actual story - beginning/middle/end, one character, ends with a natural mention of the business>`
...
const FUNNY_SCENERY_SYSTEM = `${SCENERY_SYSTEM}

This story was built on a specific stated premise (given below) - protect it. Don't soften, explain, or hedge the incongruity while editing; the scenery/visual line should visually reinforce the premise's mismatch, not just illustrate the surface plot.`
...
export async function writeAdStoryFast(brief, { tone = 'professional', writer = 'gemini' } = {}) {
  ...
  // Real connection between yesterday's tone work and today's vertical
  // routing: they aren't independent. A high-trust business (medical/
  // legal/financial) needs its comedic register reined in even under
  // Funny tone - the same baseline/reversal mechanism, just dialed down
  // to restrained wit instead of broad comedy, since looking like it's
  // trying too hard to be funny costs a business that sells invisible
  // expertise its credibility. Vertical constrains HOW tone gets
  // expressed; tone still decides WHETHER it applies.
  const verticalNote = verticalConstraints(brief.vertical).toneRegisterNote
  const draftBrief = `${briefText(brief)}\n\nWrite a short (35-45 word, fewer if it includes dialogue) real short story - not ad copy - with a beginning, middle, and end, that doubles as ad narration for this business. One clear throughline: pick ONE customer/character and follow them through one real moment (want/need -> they get it -> the payoff). End with a natural, ungimmicky mention of the business - it should feel like part of the story's own world, not a tacked-on slogan.${toneHint ? ` ${toneHint}` : ''}${verticalNote ? ` ${verticalNote}` : ''}`

  let premise = null
  let pitch
  if (isFunny) {
    const raw = await writeFn(draftBrief, FUNNY_WRITER_SYSTEM)
    const premiseMatch = raw.match(/PREMISE:\s*([\s\S]*?)\s*STORY:/i)
    const storyMatch = raw.match(/STORY:\s*([\s\S]*)/i)
    premise = premiseMatch?.[1]?.trim() || null
    pitch = storyMatch?.[1]?.trim() || raw.trim()
  } else {
    pitch = await writeFn(draftBrief, WRITE_SYSTEM)
  }

  const editPrompt = premise
    ? `Premise: ${premise}\n\nWriter's draft:\n\n${pitch}\n\nEdit and finalize now.`
    : `Writer's draft:\n\n${pitch}\n\nEdit and finalize now.`
  // Real bug caught live: this single "visual" line becomes the free
  // preview's actual image-generation prompt, exactly as visible to a
  // visitor as the full shot breakdown - but only the shot breakdown
  // (shots.js) had the vertical's people/framing guidance. A high-trust
  // business's free preview was generating "a nurse hands him a bag"
  // before this. Appended at call-time since SCENERY_SYSTEM/
  // FUNNY_SCENERY_SYSTEM are shared constants, not per-brief.
  const visualGuidance = verticalConstraints(brief.vertical).visualGuidance
  const sceneryBase = isFunny ? FUNNY_SCENERY_SYSTEM : SCENERY_SYSTEM
  const scenerySystem = visualGuidance
    ? `${sceneryBase}\n\nThe visual line is exactly as visible to the audience as the full shot breakdown elsewhere in this pipeline, so the same real constraint applies to it: ${visualGuidance}`
    : sceneryBase
  const raw = await callClaude(editPrompt, scenerySystem, 1200)
  ...
  final.trace = { mode: 'fast', tone, writer, vertical: brief.vertical || null, premise, pitch }
  return final
}
