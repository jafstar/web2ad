...
import { readSchema, createRun, writeSchema, patchShot } from './runStore.js'
...
// scripts as child processes). Here, every run gets its own id and a
// Supabase row so multiple users' ads don't collide, and actions call the
// generation functions directly in-process rather than staging a queue +
// spawning child processes - the queue existed to work around Kling's
// specific concurrency limit, which isn't our bottleneck yet.
//
// Real production bug fixed here: this used to be local-disk state under
// process.cwd()/.adbuilder-runs/<runId>/ - broke on Vercel two ways
// (process.cwd() is the deployed bundle's read-only dir, and even /tmp
// wouldn't survive the paid flow's multiple separate requests over real
// time landing on different serverless containers). See runStore.js and
// supabase/migrations/0007_adbuilder_runs.sql. Keyframe/render media now
// live on Cloudinary; schema.shots[].keyframeUrl/renderUrl hold those
...
function breakdownSystem({ maxScenes, framing, clipDuration, visualGuidance }) {
  const durationNote = {
...
export async function generateShotBreakdown(brief, script, options = {}) {
  // Vertical's suggested framing is a STRONG DEFAULT, not a hard override -
  // it only applies when the caller doesn't explicitly pass framing (e.g.
  // the wizard UI initialized its framing state from the vertical but the
  // user is free to change it; an explicit choice always wins here).
  const vertical = verticalConstraints(brief.vertical)
  const opts = { ...DEFAULT_OPTIONS, framing: vertical.suggestedFraming, ...options }
  ...
  const raw = await callClaude(prompt, breakdownSystem({ maxScenes: opts.maxScenes, clipDuration: opts.clipDuration, framing: opts.framing, visualGuidance: vertical.visualGuidance }), 1200)
...
// Downloads a Cloudinary (or any https) URL and returns it as a base64
// data URL - generateFlux's referenceImageDataUrl param expects real
// image bytes, not a URL, for reference-conditioned generation.
export async function urlToDataUrl(url, mime = 'image/jpeg') {
...
// Exported for lib/adbuilder/beatPipeline.js - the new phrase-timed
// pipeline reuses the exact same real Flux/Hailuo/Seedance/Cloudinary
// calls, just with per-beat durations instead of a fixed clipDuration
// tier. Everything else in this file (the existing 4-fixed-shot flow) is
// untouched.
export async function generateKeyframeFor(runId, shotId, sceneDescription, brief, atmosphere, fixNote = '', referenceImageDataUrl = null) {
  ...
  const { url } = await uploadBufferToCloudinary(Buffer.from(match[1], 'base64'), `adbuilder/${runId}/keyframes`, 'image')
  return { url, dataUrl }
}

export async function generateMotionFor(runId, shotId, imageDataUrl, sceneDescription, durationSeconds, fixNote = '') {
  ...
  if (durationSeconds === 30) {
    ...
    const { url } = await uploadBufferToCloudinary(buf, `adbuilder/${runId}/renders`, 'video')
    return url
  }
...
// moment its status changes in Supabase - no separate progress UI needed,
// just not blocking the response on the part that was already pollable.
export async function initializeRun(runId, brief, script, options = {}, userId = null) {
  // Real bug caught before this ever shipped: pre-resolving opts here with
  // plain DEFAULT_OPTIONS (framing: 'balanced') and THEN passing that
  // already-concrete opts object into generateShotBreakdown meant its own
  // vertical-aware default could never apply - by the time it saw
  // options.framing, it was always already set to something. Same
  // vertical-aware merge here, on the raw caller options, so an
  // unspecified framing really does fall through to the vertical's
  // suggestion instead of silently landing on 'balanced' first.
  const vertical = verticalConstraints(brief.vertical)
  const opts = { ...DEFAULT_OPTIONS, framing: vertical.suggestedFraming, ...options }
  ...
  await createRun(runId, schema, userId)
  return schema
}
...
export async function runShotGeneration(runId, brief) {
  ...
  if (opts.continuity) {
    ...
    for (const shot of shots) {
      try {
        const { url: keyframeUrl, dataUrl } = await generateKeyframeFor(runId, shot.id, shot.sceneDescription, brief, atmosphere, '', referenceImageDataUrl)
        await patchShot(runId, shot.id, { status: 'keyframe-done', keyframeUrl })
        const renderUrl = await generateMotionFor(runId, shot.id, dataUrl, shot.sceneDescription, shot.durationSeconds)
        await patchShot(runId, shot.id, { status: 'done', renderUrl, error: null })
        referenceImageDataUrl = dataUrl
      } catch (e) {
        console.error(`[adbuilder] shot ${shot.id} failed:`, e.message)
        await patchShot(runId, shot.id, { status: 'error', error: e.message })
    ...
    await Promise.all(shots.map(async (shot) => {
      try {
        const { url: keyframeUrl, dataUrl } = await generateKeyframeFor(runId, shot.id, shot.sceneDescription, brief, atmosphere)
        await patchShot(runId, shot.id, { status: 'keyframe-done', keyframeUrl })
        const renderUrl = await generateMotionFor(runId, shot.id, dataUrl, shot.sceneDescription, shot.durationSeconds)
        await patchShot(runId, shot.id, { status: 'done', renderUrl, error: null })
      } catch (e) {
...
async function findPriorReferenceImage(schema, shotId) {
  if (!schema.options?.continuity) return null
  const priorDone = schema.shots
    .filter((s) => s.id < shotId && !s.disabled && s.status === 'done' && s.keyframeUrl)
    .sort((a, b) => b.id - a.id)[0]
  if (!priorDone) return null
  return urlToDataUrl(priorDone.keyframeUrl)
}
...
export async function patchKeyframe(runId, shotId, fixNote) {
  ...
  const referenceImageDataUrl = await findPriorReferenceImage(schema, shotId)
  const { url: keyframeUrl, dataUrl } = await generateKeyframeFor(runId, shotId, shot.sceneDescription, schema.brief, schema.atmosphere, fixNote, referenceImageDataUrl)
  const renderUrl = await generateMotionFor(runId, shotId, dataUrl, shot.sceneDescription, shot.durationSeconds, fixNote)
  await patchShot(runId, shotId, { status: 'done', keyframeUrl, renderUrl, error: null })
  return readSchema(runId)
}
...
export async function patchMotion(runId, shotId, fixNote) {
  ...
  const shot = schema.shots.find((s) => s.id === shotId)
  if (!shot) throw new Error('Shot not found')
  if (!shot.keyframeUrl) throw new Error('Shot has no keyframe yet')
  const imageDataUrl = await urlToDataUrl(shot.keyframeUrl)
  const renderUrl = await generateMotionFor(runId, shotId, imageDataUrl, shot.sceneDescription, shot.durationSeconds, fixNote)
  await patchShot(runId, shotId, { status: 'done', renderUrl, error: null })
  return readSchema(runId)
}

export async function toggleShot(runId, shotId) {
  const schema = await readSchema(runId)
  const shot = schema.shots.find((s) => s.id === shotId)
  if (!shot) throw new Error('Shot not found')
  await patchShot(runId, shotId, { disabled: !shot.disabled })
  return readSchema(runId)
}
